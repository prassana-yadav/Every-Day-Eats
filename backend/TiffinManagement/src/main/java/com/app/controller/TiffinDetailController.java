...
public class TiffinDetailController {
	...
	public ResponseEntity<?> saveAlbum(TiffinFormDto tiffinDto) throws IOException {
		...
		//TiffinFromDto madhe multiPart File asel tyat actual image asel
		//database madhe only tiffin Image Nmae store karayache ahe image disk madhe store karaych ahe
		//tyasathi converter 
		TiffinDetail tiffin = converter.albumFormDtoToEntity(tiffinDto);
		//storage service iterface ahe DiskStorage madhe implement kela ahe 
		//store method madhe image disk madhe store hoil ani string return karel
		String thumbnail = storageService.store(tiffinDto.getTiffinImage());
		//set tiffin name in tiffin 
